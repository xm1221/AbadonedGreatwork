...
typealias RegistrarEntry<T> = Abadoned_greatworkRegistrar<T>.Entry<out T>

abstract class Abadoned_greatworkRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Abadoned_greatwork.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is Abadoned_greatworkRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
